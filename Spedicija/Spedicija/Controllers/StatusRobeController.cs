using System;
using System.Collections.Generic;
using System.Data;
using System.Data.Entity;
using System.Linq;
using System.Web;
using System.Web.Mvc;
using Spedicija.Models;


namespace Spedicija.Controllers
{
    public class StatusRobeController : Controller
    {
        private uvhszjiy_spedicijaEntities db = new uvhszjiy_spedicijaEntities();

        //
        // GET: /StatusRobe/
        [Authorize]
        public ActionResult Index()
        {
            // return View(db.StatusRobe.ToList());
			 return View(new List<StatusRobe>());
        }

		// 
	 public ActionResult IndexAjax(jQueryDataTableParamModel param)
	 {
		 var list = db.StatusRobe.ToList();
         IEnumerable<StatusRobe> filteredList;

				var FIdStatusRobe  = Convert.ToString(Request["sSearch_0"]).ToLower();
				var FNaziv  = Convert.ToString(Request["sSearch_1"]).ToLower();
		
        filteredList = list.Where(c =>	
				  (String.IsNullOrEmpty("" + c.IdStatusRobe ) ? "" :  "" +  c.IdStatusRobe).ToLower().Contains(FIdStatusRobe) 
				 &&   (String.IsNullOrEmpty("" + c.Naziv ) ? "" :  "" +  c.Naziv).ToLower().Contains(FNaziv) 
		                                  );


            if (!string.IsNullOrEmpty(param.sSearch))
            {
                filteredList = filteredList.Where(c =>  
											  (String.IsNullOrEmpty("" + c.IdStatusRobe ) ? "" :  "" +  c.IdStatusRobe).ToLower().Contains(param.sSearch.ToLower()) 
		
											 ||   (String.IsNullOrEmpty("" + c.Naziv ) ? "" :  "" +  c.Naziv).ToLower().Contains(param.sSearch.ToLower()) 
		
																	);

            }

            var sortColumnIndex = Convert.ToInt32(Request["iSortCol_0"]);

            Func<StatusRobe, object> orderingFunction = c =>
            {
                switch (sortColumnIndex)
                {
				 					case 0: return c.IdStatusRobe;
									case 1: return c.Naziv;
									default: return c.IdStatusRobe;
                }
			};

            var sortDirection = Request["sSortDir_0"]; // asc or desc
            if (sortDirection == "asc")
                filteredList = filteredList.OrderBy(orderingFunction);
            else
                filteredList = filteredList.OrderByDescending(orderingFunction);

            var displayedList = filteredList.Skip(param.iDisplayStart).Take(param.iDisplayLength);
            var result = from c in displayedList select new object[] { 
			 				c.IdStatusRobe,
							c.Naziv,
								""
            };
            return Json(new
            {
                sEcho = param.sEcho,
                iTotalRecords = list.Count(),
                iTotalDisplayRecords = filteredList.Count(),
                aaData = result
            },
                        JsonRequestBehavior.AllowGet);

		}


        //
        // GET: /StatusRobe/Details/5
        [Authorize]
        public ActionResult Details(int id = 0)
        {
            StatusRobe statusrobe = db.StatusRobe.Find(id);
            if (statusrobe == null)
            {
                return HttpNotFound();
            }
            return View(statusrobe);
        }

        //
        // GET: /StatusRobe/Create
        [Authorize]
        public ActionResult Create()
        {
            return View();
        }

        //
        // POST: /StatusRobe/Create

        [HttpPost]
        public ActionResult Create(StatusRobe statusrobe)
        {
            if (ModelState.IsValid)
            {
                db.StatusRobe.Add(statusrobe);
                db.SaveChanges();
                return RedirectToAction("Index");
            }

            return View(statusrobe);
        }

        //
        // GET: /StatusRobe/Edit/5
        [Authorize]
        public ActionResult Edit(int id = 0)
        {
            StatusRobe statusrobe = db.StatusRobe.Find(id);
            if (statusrobe == null)
            {
                return HttpNotFound();
            }
            return View(statusrobe);
        }

        //
        // POST: /StatusRobe/Edit/5

        [HttpPost]
        public ActionResult Edit(StatusRobe statusrobe)
        {
            if (ModelState.IsValid)
            {
                db.Entry(statusrobe).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(statusrobe);
        }

        //
        // GET: /StatusRobe/Delete/5
        [Authorize]
        public ActionResult Delete(int id = 0)
        {
            StatusRobe statusrobe = db.StatusRobe.Find(id);
            if (statusrobe == null)
            {
                return HttpNotFound();
            }
            return View(statusrobe);
        }

        //
        // POST: /StatusRobe/Delete/5

        [HttpPost, ActionName("Delete")]
        public ActionResult DeleteConfirmed(int id)
        {
            StatusRobe statusrobe = db.StatusRobe.Find(id);
            db.StatusRobe.Remove(statusrobe);
            db.SaveChanges();
            return RedirectToAction("Index");
        }







        protected override void Dispose(bool disposing)
        {
            db.Dispose();
            base.Dispose(disposing);
        }
    }
}